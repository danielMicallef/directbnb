...
def configure_custom_domain(
    project_name: str, domain: str, base_url: str, headers: dict
) -> dict:
    """
...
def deploy_to_cloudflare_pages(
    ...
    files = {"file": ("deployment.zip", zip_bundle, "application/zip")}
...
def export_property_to_theme(property_obj: Property, lead: LeadRegistration) -> Path:
    ...
    property_data = {
        ...
        if hasattr(property_obj, "coordinates")
        else None,
        # Host
        "host": {
