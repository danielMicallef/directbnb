...
class Listing(AbstractTrackedModel):
    ...
    platform = models.CharField(
        max_length=255, choices=PlatformChoices.choices, default=PlatformChoices.AIRBNB
    )
...
class Property(AbstractTrackedModel):
    ...
    lead = models.ForeignKey(
        LeadRegistration, on_delete=models.CASCADE, null=True, blank=True
    )
...
class TouristAttraction(AbstractTrackedModel):
    ...
    distance = models.CharField(
        max_length=100, help_text="e.g., '2 km away', '10 min walk'"
    )
    category = models.CharField(
        max_length=100, help_text="e.g., 'Restaurant', 'Museum', 'Beach', 'Park'"
    )
    image = models.ImageField(upload_to="attractions/", blank=True, null=True)
    image_url = models.URLField(
        blank=True, null=True, help_text="Fallback if image not uploaded"
    )
